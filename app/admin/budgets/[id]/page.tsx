...
const BudgetDetailsPage = async ({ params }: { params: Promise<{ id: string }> }) => {
  const { id } = await params;
  const budget = await getBudget(id);
  ...
  return (
    <>
      <div className='flex justify-between items-center'>
        <div>
          <h1 className="font-black text-4xl text-purple-950 mb-1">{budget.name}</h1>
      ...
      {budget.expenses.length ? (
        <>
          ...
          <h1 className="text-3xl font-black text-purple-950 mt-10">Gastos en este presupuesto:</h1>
        ...
        <p className="text-center py-20">Aún no hay gastos</p>
      )
      }
